...
@InitiatingFlow
@StartableByRPC
public class SanctionPurchaseOrder extends FlowLogic<SignedTransaction> {
    private int index = 0;
    ...
    public SanctionPurchaseOrder(String identifier) {
    ...
    public SignedTransaction call() throws FlowException {
        ...
        PurchaseOrderState outputState = new PurchaseOrderState(
                ...
                PurchaseOrderStatus.PaidByLender.toString(),
                inputStateStateAndRef.getState().getData().getAmountPaid(),
                inputStateStateAndRef.getState().getData().getInvoices(), buyer, seller, getOurIdentity(),
                inputStateStateAndRef.getState().getData().getMonthlyEMI(),
                inputStateStateAndRef.getState().getData().getTotalPayment());

        Command command = new Command(new PurchaseOrderContract.SanctionAmount(), getOurIdentity().getOwningKey());
